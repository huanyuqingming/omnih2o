...
motion_file = "/home/szsh/ztj/human2humanoid/legged_gym/resources/motions/h1/amass_phc_filtered.pkl"
# motion_file = "/home/szsh/ztj/human2humanoid/legged_gym/resources/motions/h1/stable_punch.pkl"
...
while not gym.query_viewer_has_closed(viewer):
    ...
    for motion_id in range(num_motions):
        ...
        root_poss.append(root_pos)
        root_rots.append(root_rot)
        dof_poss.append(dof_pos)
        root_vels.append(root_vel)
        root_ang_vels.append(root_ang_vel)
        dof_vels.append(dof_vel)
        smpl_paramss.append(smpl_params)
        limb_weightss.append(limb_weights)
        pose_aas.append(pose_aa)
        rb_poss.append(rb_pos)
        rb_rots.append(rb_rot)
        body_vels.append(body_vel)
        body_ang_vels.append(body_ang_vel)

        if args.show_axis:
            gym.clear_lines(viewer)

    root_poss = torch.stack(root_poss).squeeze().to(device)
    root_rots = torch.stack(root_rots).squeeze().to(device)
    dof_poss = torch.stack(dof_poss).squeeze().to(device)
    root_vels = torch.stack(root_vels).squeeze().to(device)
    root_ang_vels = torch.stack(root_ang_vels).squeeze().to(device)
    dof_vels = torch.stack(dof_vels).squeeze().to(device)
    smpl_paramss = torch.stack(smpl_paramss).squeeze().to(device)
    limb_weightss = torch.stack(limb_weightss).squeeze().to(device)
    pose_aas = torch.stack(pose_aas).squeeze().to(device)
    # rb_poss = torch.stack(rb_poss).squeeze().to(device)
    rb_rots = torch.stack(rb_rots).squeeze().to(device)
    body_vels = torch.stack(body_vels).squeeze().to(device)
    body_ang_vels = torch.stack(body_ang_vels).squeeze().to(device)
    ...
    for i in range(num_envs):
        ...
        for pos_joint in rb_pos[0, 1:]: # idx 0 torso (duplicate with 11)
            ...
            gymutil.draw_lines(sphere_geom2, gym, viewer, envs[i], sphere_pose) 
    # import pdb; pdb.set_trace()
    ...
    ###########################################################################
    # root_states = torch.cat([root_pos, root_rot, root_vel, root_ang_vel], dim=-1).repeat(num_envs, 1)   # [num_envs, 13]
    root_states = torch.cat([root_poss, root_rots, root_vels, root_ang_vels], dim=-1)   # [num_envs, 13]
    ...
    # dof_state = torch.stack([dof_pos, torch.zeros_like(dof_pos)], dim=-1).squeeze().repeat(num_envs, 1) # [num_envs * 19, 2]
    dof_state = torch.stack([dof_poss, torch.zeros_like(dof_poss)], dim=-1).reshape(-1, 2)  # [num_envs * 19, 2]
